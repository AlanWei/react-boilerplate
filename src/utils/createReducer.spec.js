...
test('default state should be a function', () => {
  expect(() => createReducer()).toThrowError(
    '[createReducer] defaultState should be a function',
  );
});
...
it('should return current state when action is not an object', () => {
...
test('should return a original state when no handler found', () => {
  const reducer = createReducer(defaultState);
  expect(reducer(undefined, { type: 'NOTFOUND' })).toEqual(defaultState());
});

test('should call handler when match', () => {
